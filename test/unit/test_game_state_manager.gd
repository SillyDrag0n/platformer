...
# completing one bounty auto-unlocks any bounty that names it as a prerequisite, e.g. the Cactus
# Coyote contract unlocking Sand Spirit. Tests that mutate GameStateManager.bounties directly
# (real singleton state) use throwaway ids and restore the array + lookup cache afterward.

const TUTORIAL_BOUNTY_ID := "missing_cattle"
const SAND_SPIRIT_BOUNTY_ID := "1"


func test_sand_spirit_requires_the_first_bounty_and_starts_locked():
	...
	assert_false(sand_spirit.unlocked, \
		"Sand Spirit should start locked until the Missing Cattle contract is finished")
